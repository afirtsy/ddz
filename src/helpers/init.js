...
export function initializeParams(request, env) {
    ...
    if (globalThis.pathName !== '/secrets') {
        if (!globalThis.userID || !globalThis.TRPassword) throw new Error(`Please set UUID and ${atob('VHJvamFu')} password first. Please visit <a href="https://${globalThis.hostName}/secrets" target="_blank">here</a> to generate them.`, { cause: "init"});
